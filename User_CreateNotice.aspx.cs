...
{
    public partial class User_CreateNotice : System.Web.UI.Page
    {
        ...
        (Title, Description, Posted_date, Expiry_date, File_path, Notice_type, Importance, Status ,Broadcast_By,Send_via,User_id)
        VALUES
        (@Title, @Description, GETDATE(), @ExpiryDate, @FilePath, @NoticeType, @Importance, @Status,@broad,@send,@uid);
        SELECT SCOPE_IDENTITY();";
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@uid", Session["U_id"] ?? DBNull.Value);
                    conn.Open();
        ...
        private void ShowSuccess(string message)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "SuccessMessage",
                $"Swal.fire('Success!', '{message.Replace("'", "\\'")}', 'success').then(() => window.location = 'User_CreateNotice.aspx');",
                true);
        }
